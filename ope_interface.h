...
class OPE_Interface {
	private:
		const ZZ tau, n;
	public:
		OPE_Interface(ZZ tau, int log2T);
		const ZZ& getTau() const;
		const ZZ& getN() const;
		const Vec<long> generateAlpha() const;
		// Generate random polynomial set R
		const Vec<Vec<ZZ_p>> generateR(Poly_Field& PF, Vec<long>& alpha) const;	
		const Vec<ZZ_p> hRP(FirstDegPolynomial& Poly, Vec<Vec<ZZ_p>>& R, Vec<long>& alpha) const;
		const Vec<ZZ_p> generateVector_r() const;
		const Vec<Vec<ZZ_p>> PreparePairs(Vec<ZZ_p> r, Vec<Vec<ZZ_p>> R, ZZ_p x_star) const;
