...
const ZZ_p OLE_enhanced::runOLE_plus(ZZ x_star, const Vec<ZZ>& coeff_ab, const Vec<ZZ>& secrets_su) const {
	...
	std::cout << "Correct t: " << (to_ZZ_p(secrets_su[1])*x_inverse + to_ZZ_p(secrets_su[0])) << std::endl;
	...
	std::cout << "Correct c: " << (masked_coeff[0] + masked_coeff[1] * x_star_ZZ_p) << std::endl;
	std::cout << "Computed c: " << c << std::endl;
