...
const ZZ_p OLE_enhanced::runOLE_plus(ZZ x_star, const Vec<ZZ>& coeff_ab, const Vec<ZZ>& secrets_su) {
        // Field and Polynomial Setup
	int key_bits = NumBits(this->p);
	
	// Initialise base OLE
	OLE OLE_p = OLE(this->p, key_bits, PFv.back());
...
// overload, if one wants to reuse under a new field
const ZZ_p OLE_enhanced::runOLE_plus(ZZ x_star, const Vec<ZZ>& coeff_ab, const Vec<ZZ>& secrets_su, const Poly_Field PF) {
        // Field and Polynomial Setup
	PFv.push_back(PF);
        int key_bits = NumBits(this->p);
        // Initialise base OLE
...
const Poly_Field OLE_enhanced::getPF() const {return PFv.back();};
